...
class fancourier extends \Opencart\System\Engine\Controller {
    private $error = array();

    public function index() {
        $this->load->model('integration/delivery/fancourier');

        $this->load->language('integration/fancourier');

        $this->document->setTitle($this->language->get('heading_title'));

        $storage = $this->model_integration_delivery_fancourier->getStorage($this->user->getId());

        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateSettingForm()) {
            $storage = $this->request->post;

            $this->model_integration_delivery_fancourier->editStorage($this->user->getId(), $storage);

            if ($this->model_integration_delivery_fancourier->connect($this->user->getId(), true)) {
        ...
        $data['action'] = $this->url->link('integration/delivery/fancourier', 'user_token=' . $this->session->data['user_token'], true);

        $data['header'] = $this->load->controller('common/header');
        $data['footer'] = $this->load->controller('common/footer');
        $data['navigation'] = $this->load->controller('common/navigation');

        $this->response->setOutput($this->load->view('integration/fancourier', $data));
    }
    ...
    public function waybill() {
        ...
        $connect = $this->model_integration_delivery_fancourier->connect($this->user->getId());

        if (!$connect) {
            $this->session->data['error'] = $this->language->get('error_connect');
            $this->response->redirect($this->url->link('integration/delivery/fancourier', 'user_token=' . $this->session->data['user_token'], true));
        }
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateWaybillForm()) {
            ...
            $result = $this->model_integration_delivery_fancourier->createShipment($post);

            if (isset($result['id']) and isset($result['price'])) {
                $this->model_sale_order->editTracking($this->user->getId(), $order_id, 'fancourier', $result['id']);
        ...
        $data['services'] = $this->model_integration_delivery_fancourier->getServices();

        $data['services_json'] = json_encode($data['services']);

        $data['clients'] = $this->model_integration_delivery_fancourier->getContractClients();
        ...
        if ($data['country_name'] == '' and $order_info['shipping_country']) {
            $countries = $this->model_integration_delivery_fancourier->getCountries($order_info['shipping_country']);
            ...
            if (is_array($countries) and count($countries) == 1) {
                ...
                if ($countries[0]['siteNomen'] == 0) {
                    ...
                    $sites = $this->model_integration_delivery_fancourier->getSites($countries[0]['id'], $order_info['shipping_city']);
        ...
        $data['action'] = $this->url->link('integration/delivery/fancourier.waybill', $url, true);
        ...
        $this->response->setOutput($this->load->view('integration/fancourier_wb_form', $data));
    }
    ...
    public function getCountries() {
        ...
        if (!empty($this->request->get['search'])) {
            ...
            $countries = $this->model_integration_delivery_fancourier->getCountries($this->request->get['search']);
    ...
    public function getStates() {
        ...
        if (!empty($this->request->get['search']) and !empty($this->request->get['country_id'])) {
            ...
            $states = $this->model_integration_delivery_fancourier->getStates($this->request->get['country_id'], $this->request->get['search']);
    ...
    public function getSites() {
        ...
        if (!empty($this->request->get['search']) and !empty($this->request->get['country_id'])) {
            ...
            $sites = $this->model_integration_delivery_fancourier->getSites($this->request->get['country_id'], $this->request->get['search']);
    ...
    public function getStreets() {
        ...
        if (!empty($this->request->get['search']) and !empty($this->request->get['site_id'])) {
            ...
            $streets = $this->model_integration_delivery_fancourier->getStreets($this->request->get['site_id'], $this->request->get['search']);
    ...
    public function print () {
        if (isset($this->request->get['tracking'])) {
            ...
            $result = $this->model_integration_delivery_fancourier->getLabel($this->request->get['tracking']);
