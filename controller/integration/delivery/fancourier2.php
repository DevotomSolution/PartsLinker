...
class fancourier extends \Opencart\System\Engine\Controller {
    ...
    public function index() {
        ...
        $storage = $this->model_integration_delivery_delivery->getStorage($this->user->getId(), 'fancourier');

        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateSettingForm()) {
            $storage = $this->request->post;

            $this->model_integration_delivery_delivery->editStorage($this->user->getId(), 'fancourier', $storage);
